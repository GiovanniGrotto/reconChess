...
class TroutBot(Player):
    ...
    def handle_opponent_move_result(self, captured_my_piece: bool, capture_square: Optional[Square]):
        print("### INIZIO TURNO SYNTHOL CONTRO " + self.opponent + " ###", end='\n')
    ...
    def prova(self):
        board = chess.Board("4k3/2npp3/8/8/8/8/2Q2N2/3K4 w - - 0 1")
        analysis = self.engine.analyse(board, chess.engine.Limit(depth=DEPTH), multipv=MULTIPV)
        if self.color == True:
                eval = [move["score"].white() for move in analysis]
        else:
                eval = [move["score"].black() for move in analysis]
        for i in range(len(eval)):
            print(eval[i])
            if "#" in str(eval[i]):
                eval[i] = eval[i].score(mate_score=1000000)
            else :
                eval[i] = eval[i].score()
            print(eval[i])
        return None
    ...
    def handle_sense_result(self, sense_result: List[Tuple[Square, Optional[chess.Piece]]]):
        ...
        # Found both from where and to where the piece was from
        ...
        # Found just where the piece arrived, not where it came from
            ...
            move = self.get_move_to_tile(self.board, to_tile, found_piece)
        ...
        # Found just where the piece came from, not where arrived
            ...
            self.top5Move = moves_from_tile
            #ora bisogna calcolare la matrice con queste mosse
            self.uncertanty = True
            if len(self.top5Move) > 0 : move = self.top5Move[0]
            else : move = None
        ...
        print("Mossa di " + self.opponent + ": "+ str(move), end='\n')
        self.f.write("Mossa di " + self.opponent + ": "+ str(move) + "\n")
        if move != None:
    ...
    def choose_move(self, move_actions: List[chess.Move], seconds_left: float) -> Optional[chess.Move]:
        ...
        if enemy_king_square:
            ...
            if enemy_king_attackers:
                ...
                print("Mossa Synthol: "+ str(move), end='\n')
                self.f.write("Mossa Synthol: "+ str(move) + "\n")
                return move
        
        # durante il sensing non ho trovato nulla, devo gestire l'incertezza con la matrice
        if  self.uncertanty : 
            self.uncertanty = False
            best_response_move = self.get_best_response_move(self.top5Move)
            if best_response_move is not None:
                move = best_response_move
                print("Mossa Synthol: "+ str(move), end='\n')
                self.f.write("Mossa Synthol: "+ str(move) + "\n")
                return move
    ...
    def handle_move_result(self, requested_move: Optional[chess.Move], taken_move: Optional[chess.Move],
        ...
        print("Secondi rimanenti: "+ str(self.second_left))
        self.f.write("Secondi rimanenti: "+ str(self.second_left) + "\n\n")
        print("### FINE TURNO SYNTHOL CONTRO " + self.opponent + " ###", end='\n')
        print()
...
####################################### CHOOSE SENSE FUNCTIONS #######################################
    ...
    def stockfish_tile_weight(self):
        ...
        analysis = self.engine.analyse(self.board, chess.engine.Limit(depth=DEPTH), multipv=MULTIPV)
...
####################################### MANAGE SENSE FUNCTIONS #######################################
    ...
    # TODO: se due pezzi mirano alla stessa prendi il più vicino in mosse
    # Knight: https://math.stackexchange.com/questions/3270282/chess-board-knight-distance
    # Bishop: capisci che colore è e togli quello dello stesso colore
    # Queen,King : sono singoli
    # Rook: ????
    # Pound: elimina il pedone migliore da muovere
    def get_move_to_tile(self, board, to_tile, found_piece):
        found_startings = []
        for tile in range(0, 64):
            new_piece = board.piece_at(tile)
            if found_piece == new_piece :
                #print("Trovata partenza " + str(tile))
                if tile != to_tile:
                    found_startings.append(tile)
                move = self.intToNotation(tile) + self.intToNotation(to_tile)
                if self.board.is_pseudo_legal(chess.Move.from_uci(move)):
                    return move
        min_start = None
        if len(found_startings)>0 : 
            min_start = found_startings[0]
            for found_starting in found_startings:
                if chess.square_distance(to_tile, found_starting) < chess.square_distance(to_tile,min_start):
                    min_start = found_starting
        
        self.board.set_piece_at(to_tile, found_piece)
        if min_start != None : self.board.remove_piece_at(min_start)

        return None

    # Getting a tile gives a list of all the moves leaving from that tile, and arriving in non-sensed tile
    def get_moves_from_tile(self, from_tile):
        moves_from_tile = []
        for move in self.board.pseudo_legal_moves: #possono essere state fatte anche mosse che lasciano il re sotto scacco, è importante saperlo
    ...
    def generatePossibleMoves(self, matrix, moves):
        ...
        for m in moves:
            ...
            for t in top:
                ...
                response.append(t)
                matrix[y][x] = eval[idx]
                dict_of_moves[t] = eval[idx]
                y += 1
                idx += 1
            dict_of_top5[m] = dict_of_moves.copy()
            dict_of_moves.clear()
            self.board.pop()
            x += 1
        #self.board.push(old_move)
        return matrix, response, dict_of_top5

    # Fill the matrix and calculate the maxmin value, corresponding at the best move on worst case
    def fullFillMatrix(self, matrix, moves, response, dictOfMoves):
        minimum = -1000000
        minIndex = 0
        moves = list(moves)
        ...
        for m in response:
            ...
            for t in moves:
                self.board.push(chess.Move.from_uci(t))
                ...
                if e < minimum:
                    ...
                    matrix[x][y] = e
                    self.board.pop()
                    self.board.pop() #questo va fatto perchè in questo if ho pushato move a riga 491
                    break
                lineMinValue = min(lineMinValue, e)
                matrix[x][y] = e
                self.board.pop()
        ...
        print(minIndex)
        print(minimum)
        print(response[minIndex])"""
        self.board.push(chess.Move.from_uci(moves[0])) #ora sono nella situazione in cui tocca al nero e ancora non ha mosso, serviva per la simulazione, adesso però devo assegnargli una mossa prima di fare la mia
        return matrix, response[minIndex]
    ...
    def evaluation_matrix(self, list_of_move):
        ...
        matrix.fill(np.nan)
        #print(list_of_move)
        matrix, list_of_response, dict_of_moves = self.generatePossibleMoves(matrix, list_of_move)
        #print(matrix)
        matrix, move = self.fullFillMatrix(matrix,list_of_move, list_of_response, dict_of_moves)
        return move
    ...
    def get_best_response_move(self, moves):
        ...
        if len(moves) == 1:
            ...
            return chess.Move.from_uci(self.evaluation_matrix(moves))
